...
namespace SMSManager
{
    public class clsSMS
    {
        ...
        public string GetInfoNumberAndNetWork(SerialPort port)
        {
            try
            {

              
                string command = "AT+CUSD=1,\"*0#\",15";
                string recievedData = ExecCommand(port, command, 5000, "Failed to send message"); //3 seconds
                string command2 = "AT+COPS?";
                recievedData += ExecCommand(port, command2, 5000, "Failed to send message"); //3 seconds
                return recievedData;
            }
            catch (Exception ex)
            {
